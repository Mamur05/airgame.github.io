...
# Ищем пополнения за последние 60 дней
$tlast = time()-60*60*24*600;
$tnow = time();

$last_in = $db->query('SELECT SUM(sum) FROM db_insert WHERE `add` >= '.$tlast.' AND `add` <= '.$tnow.' AND status = 1 AND `uid` = '.$uid.'')->fetchArray();
$last_insert = round($last_in['SUM(sum)'],0); // Общая сумма пополнений
?>
...
$array = array(
	...
	'comment' => iconv('utf-8', 'utf-8', "Выплата пользователю {$login} с проекта ".$_SERVER["HTTP_HOST"])
);
...
}else echo '<div class="alert alert-danger">Для заказа выплат, необходима сумма пополнений за последние 60 дней!<br/><b>На данный момент Ваши пополнения '.$last_insert.' руб. Необходимая сумма минимум '.$accPay.' руб.</b></div>';
