...
if(strlen($title) >= 3){
		$db->Query("INSERT INTO db_news (title, url, img, text, `add`) VALUES ('$title','$url','$img','$text','".time()."')");
...
<b>Заголовок:</b>
<input class="form-control mb-2" type="text" name="title" value="<?=(isset($_POST['title'])) ? $_POST['title'] : false; ?>" id="title" onKeyUp="translit()" />
<b>Название ссылки:</b>
<input class="form-control form-control-sm mb-2 bg-light" type="text" name="url" id="url" value="" />
<b>Картинка для новости:</b>
<input class="form-control mb-2" type="text" name="img" size="255" value="Ссылка начинается с https://" />
<b>Новость:</b>
...
if($db->NumRows() != 1){ echo '<div class="alert alert-danger">Новость с таким ID не найдена</div>'; return;}
	...
	$text = $_POST["ntext"];
	$db->Query("UPDATE db_news SET title = '$title', url = '$url', img = '$img', text = '$text' WHERE id = '$id_edit'");
	$db->Query("SELECT * FROM db_news WHERE id = '$id_edit' LIMIT 1");
...
<form action="" method="post">
<b>Заголовок:</b>
<input class="form-control mb-2" type="text" name="title" size="45" value="<?=$news['title']; ?>" id="title" onKeyUp="translit()" />
<b>Название ссылки:</b>
<input class="form-control form-control-sm mb-2 bg-light" type="text" name="url" id="url" value="<?=$news['url']; ?>" />
<b>Картинка для новости:</b>
<input class="form-control mb-2" type="text" name="img" size="255" value="<?=$news['img']; ?>" />
